...
//更新商品信息
		public void updateGoods(Goods goods){
			...
			for (Goods querygoods2 : list) {
				if(goods.getGtype() !=null && !goods.getGtype().equals("")){querygoods2.setGtype(goods.getGtype());}
			if(goods.getGname() !=null && !goods.getGname().equals("")){querygoods2.setGname(goods.getGname());}
			if(goods.getGgrade() !=null && !goods.getGgrade().equals("")){querygoods2.setGgrade(goods.getGgrade());}
			if(goods.getGdescribe() !=null && !goods.getGdescribe().equals("")){querygoods2.setGdescribe(goods.getGdescribe());}
			if(goods.getGprice() !=null && !goods.getGprice().equals("")){querygoods2.setGprice(goods.getGprice());}
			if(goods.getGimage() !=null && !goods.getGimage().equals("")){querygoods2.setGimage(goods.getGimage());}
			if(goods.getGdate() !=null && !goods.getGdate().equals("")){querygoods2.setGdate(goods.getGdate());}
			if(goods.getGthumb() !=null && !goods.getGthumb().equals("")){querygoods2.setGthumb(goods.getGthumb());}
			if(goods.getGcomments() !=null && !goods.getGcomments().equals("")){querygoods2.setGcomments(goods.getGcomments());}
			if(goods.getGsearch() !=null && !goods.getGsearch().equals("")){querygoods2.setGsearch(goods.getGsearch());}
...
//保存订单
		public boolean savaorder(OrderAndItems orderdata) {
			...
			List<User> list=dao.query(sql, values);
			if(list.size()==0){return Successful;}
			//查出用户，级联保存订单信息
			for (User user : list) {
